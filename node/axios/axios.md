...
```js
let cancel;//用于保存取消请求的函数的变量
axios({
    url:"xxxxxxxxx",
    cancelToken: new axios.CancelToken(function executor(c){
        /*
        	executor:执行器，cancelToken实例化时调用
        	调用执行器是传入参数 c , c是一个函数类型的参数，通过外部变量保存起来，以便在需要取消请求时调用
        */
        cancel=c;
    })
})
cancel();//只有在请求成功前调用才可以取消，如果请求结果已经收到（包括错误情况），取消将没有意义，也不会被执行
//请求被取消会执行响应拦截的失败流程，最终会执行请求回调的失败处理回调
```
...
axios.interceptors.response.use(response=>{
 //响应回调
    ...
    if(axios.isCancel(error)){//判断失败结果是不是因为取消请求而失败
        //如果因为取消请求而执行失败，不能将cancel设置为null,具体原因请分析任务执行流程
        
        //中断请求，不继续向下传递错误，中断后续执行
        return new Promise(()=>{});//执行器不能少
    }else{
        cancel = null;
        return error;
    }
